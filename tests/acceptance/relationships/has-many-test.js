...
class TestAdapter extends JSONAPIAdapter {
  ...
  shouldBackgroundReload() { return false; }
  ...
  let people = [
    ...
    },
    {
      ...
      relationships: {
        children: {
          data: [
            { type: 'person', id: '3:has-2-children-and-parent' }
          ]
        },
        parent: { data: null }
      }
...
module('async has-many rendering tests', function(hooks) {
  ...
  module('for data-no-link scenarios', function() {
    ...
    test('Rendering an async hasMany whose fetch fails does not trigger a new request', async function(assert) {
      ...
      // this runs twice due to promise forking within ember-data for findRecord vs the relationship findMany which are separate things
      let originalOnError = Ember.onerror;
      Ember.onerror = function(e) {
        assert.ok(true, 'Children promise did reject');
        assert.equal(e.message, 'hard error while finding <person>5:has-parent-no-children', 'Rejection has the correct message');
      };
    ...
    `);
      ...
      assert.deepEqual(names, ['Selena has a parent', ''], 'We rendered the names');
      ...
      assert.equal(relationshipState.relationshipIsStale, false, 'The relationship is not stale');
      assert.equal(relationshipState.hasRelatedResources, false, 'The relationship is missing some or all related resources');
      assert.equal(relationshipState.hasAnyRelationshipData, true, 'The relationship knows which record it needs');
      assert.equal(relationshipState.linkPromise === null, true, 'The relationship does not have a linkPromise');
      assert.equal(relationshipState._loadingPromise !== null, true, 'The relationship has a loadingPromise');
  ...
  module('for link-no-data scenarios', function() {
    ...
    test('Rendering an async hasMany with a link whose fetch fails does not trigger a new request', async function(assert) {
      ...
      Ember.onerror = function(e) {
        assert.ok(true, 'Children promise did reject');
        assert.equal(e.message, 'hard error while finding link ./person/3:has-2-children-and-parent/children', 'Rejection has the correct message');
      };
    ...
    `);
      ...
      assert.equal(relationshipState.isAsync, true, 'The relationship is async');
      assert.equal(relationshipState.relationshipIsEmpty, true, 'The relationship is empty because no signal has been received as to true state');
      assert.equal(relationshipState.relationshipIsStale, false, 'The relationship is not stale');
      assert.equal(relationshipState.hasRelatedResources, false, 'The relationship is missing some or all related resources');
      assert.equal(relationshipState.hasAnyRelationshipData, false, 'The relationship knows which record it needs');
      assert.equal(relationshipState.linkPromise !== null, true, 'The relationship has a linkPromise');
      assert.equal(relationshipState._loadingPromise !== null, true, 'The relationship has a loadingPromise');
  ...
  module('for link-and-data scenarios', function() {
    /*
    test('We can render an async hasMany with a link', function() {

    });

    test('Re-rendering an async hasMany with a link does not cause a new fetch', function() {

    });

    test('Rendering an async hasMany with a link whose fetch fails does not trigger a new request', function() {
